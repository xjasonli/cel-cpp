...
std::vector<cel::FunctionOverloadReference> Resolver::FindOverloadsByTypes(
  ...
  auto names = FullyQualifiedNames(name, expr_id);
  for (auto it = names.begin(); it != names.end(); it++) {
    funcs = function_registry_.FindStaticOverloadsByTypes(*it, receiver_style, types);
    if (!funcs.empty()) {
      return funcs;
    }
  }
...
absl::optional<cel::FunctionOverloadReference> Resolver::FindOverloadById(
  ...
  for (const auto& prefix : prefixes) {
    std::string qualified_name = absl::StrCat(prefix, name);
    auto result = function_registry_.FindStaticOverloadById(qualified_name, overload_id);
    if (result.has_value()) {
      return result;
    }
  }
...
std::vector<cel::FunctionRegistry::LazyOverload> Resolver::FindLazyOverloadsByTypes(
    absl::string_view name, bool receiver_style,
    const std::vector<cel::Type>& types, int64_t expr_id) const {
  std::vector<cel::FunctionRegistry::LazyOverload> funcs;
  auto names = FullyQualifiedNames(name, expr_id);
  for (const auto& name : names) {
    funcs = function_registry_.FindLazyOverloadsByTypes(name, receiver_style, types);
    if (!funcs.empty()) {
      return funcs;
    }
  }
...
absl::optional<cel::FunctionRegistry::LazyOverload> Resolver::FindLazyOverloadById(
    absl::string_view name, absl::string_view overload_id) const {
  // Try with namespace prefixes
  auto prefixes = GetPrefixesFor(name);
  for (const auto& prefix : prefixes) {
    std::string qualified_name = absl::StrCat(prefix, name);
    auto result = function_registry_.FindLazyOverloadById(qualified_name, overload_id);
    if (result.has_value()) {
      return result;
    }
  }
