  ...
  void AddResolvedFunctionStep(const cel::CallExpr* call_expr,
    ...
    if (!overload_ids.empty()) {
      ...
      if (!lazy_overloads.empty()) {
        // Create lazy function step (checked = true)
        auto depth = RecursionEligible();
        if (depth.has_value()) {
      ...
      if (!static_overloads.empty()) {
        // Create static function step (checked = true)
        auto recursion_depth = RecursionEligible();
        if (recursion_depth.has_value()) {
          ABSL_DCHECK(program_builder_.current() != nullptr);
          auto args = program_builder_.current()->ExtractRecursiveDependencies();
          SetRecursiveStep(CreateDirectFunctionStep(
                  expr->id(), *call_expr, std::move(args),
      ...
      // This can happen when functions are registered without overload_id (legacy).
      // Fall through to arity-based matching as a compatibility layer.
    }
    ...
    // Try static functions
    auto overloads = resolver_.FindOverloads(
        function, receiver_style, num_args, expr->id());
    if (overloads.empty()) {
